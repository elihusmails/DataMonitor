...
public class ChartSensorPanel extends AbstractSensorPanel {
	...
	private final Color[] colors = { Color.red, Color.GREEN, Color.YELLOW,
			Color.BLUE, Color.CYAN, Color.BLUE, Color.ORANGE, Color.PINK };
	...
	public ChartSensorPanel(String t, ArrayList<String> metadata) {
		...
		// XXX: Doing some really inefficient GUI layout here, probably want to
		// fix sometime

		jfreechart = new JFreeChart(title, new Font("SansSerif", 1, 16),
				xyplot, true);
	...
	public void receiveUpdate(SensorData data) {
		...
		if (ts != null)
			ts.addOrUpdate(new Millisecond(new Date(data.getTime())),
					data.getData());
		// else {}
