...
public class DataMonitorProgressBar extends JProgressBar
{
  ...
  private void drawIndicator(Graphics g)
  {
    ...
    yPoints[1] = h;
    xPoints[2] = ox
        + ((int) (((double) w / 2) * ((double) this.getValue()) / ((double) (this.getMaximum() - this.getMinimum()))));
    ...
    if (getOtherValue() >= 0) {
      ...
      yPoints[1] = h;
      xPoints[0] = xPoints[1];
      xPoints[2] = ((int) (ox - ((double) ox) * ((double) this.getOtherValue())
  ...
  private void drawCircular(Graphics g)
  {
    ...
    g.setColor(this.getForeground());
    g.fillArc((int) bw, (int) bh, w - 2 * (int) bw, h - 2 * (int) bh, (int) a, (int) (360 - a));
